...
from utils.config_engine import (
    ...
    render_sidebar_params, compute_spectral_radius, eval_derived,
...
with st.sidebar.form("sim_cfg"):
    ...
    with st.expander("Simulation Parameters", expanded=True):
        model_choice = st.selectbox("Model Type", list(BUILTINS.keys()), index=1)
    ...
    uploaded_config = st.file_uploader("Upload custom model config (JSON)", type="json")

    if uploaded_config is not None:
        config = load_model_config_from_json_bytes(uploaded_config.read())
    else:
        config = load_model_config_from_file(BUILTINS[model_choice])

    # Model Parameters
    with st.expander("Model Parameters", expanded=False):
        param_values = render_sidebar_params(config)

    # Initial Conditions
    with st.expander("Initial Conditions", expanded=False):
        initial_infected_v = st.number_input("Initial Infected", 1, 100000, 10)
        if "Recovered" in config["compartments"]:
            initial_immunity_percentage_v = st.number_input("Initial Immunity (%)", 0.0, 100.0, 0.0, 1.0)
